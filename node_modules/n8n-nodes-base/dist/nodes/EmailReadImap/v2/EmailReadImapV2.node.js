...
exports.parseRawEmail = exports.EmailReadImapV2 = void 0;
    ...
    async trigger() {
        ...
        const establishConnection = () => {
            ...
            const config = {
                ...
                onupdate: async (seqno, info) => {
                    n8n_workflow_1.LoggerProxy.verbose(`Email Read Imap:update ${seqno}`, info);
                },
            };
            const tlsOptions = {};
            if (credentials.allowUnauthorizedCerts === true) {
            ...
            return (0, imap_simple_1.connect)(config).then(async (conn) => {
                conn.on('close', async (hadError) => {
                    if (isCurrentlyReconnecting === true) {
                        n8n_workflow_1.LoggerProxy.debug(`Email Read Imap: Connected closed for forced reconnecting`);
                    }
                    else if (closeFunctionWasCalled === true) {
                        n8n_workflow_1.LoggerProxy.debug(`Email Read Imap: Shutting down workflow - connected closed`);
                    }
                    else {
                        n8n_workflow_1.LoggerProxy.error(`Email Read Imap: Connected closed unexpectedly`);
                        this.emitError(new Error('Imap connection closed unexpectedly'));
                    }
                });
                conn.on('error', async (error) => {
                    const errorCode = error.code.toUpperCase();
                    n8n_workflow_1.LoggerProxy.verbose(`IMAP connection experienced an error: (${errorCode})`, { error });
                    await closeFunction();
                    this.emitError(error);
                });
                return conn;
        ...
        if (options.forceReconnect !== undefined) {
            reconnectionInterval = setInterval(async () => {
                n8n_workflow_1.LoggerProxy.verbose(`Forcing reconnect to IMAP server`);
                try {
                    isCurrentlyReconnecting = true;
                    if (connection.closeBox)
                        connection.closeBox(false);
                    connection.end();
                    connection = await establishConnection();
                    await connection.openBox(mailbox);
                }
                catch (error) {
                    n8n_workflow_1.LoggerProxy.error(error);
                }
                finally {
