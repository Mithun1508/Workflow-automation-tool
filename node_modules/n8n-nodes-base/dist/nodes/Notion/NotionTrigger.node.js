...
class NotionTrigger {
        ...
        this.methods = {
            loadOptions: {
                async getDatabases() {
                    var _a;
                    const returnData = [];
                    const { results: databases } = await GenericFunctions_1.notionApiRequest.call(this, 'POST', `/search`, {
                        page_size: 100,
    ...
    async poll() {
        ...
        if (Object.keys(data[0]).length !== 0 && webhookData.lastRecordProccesed !== data[0].id) {
            ...
            } while (!(0, moment_1.default)(records[records.length - 1][sortProperty]).isSameOrBefore(startDate) &&
                hasMore === true);
            if (this.getMode() !== 'manual') {
