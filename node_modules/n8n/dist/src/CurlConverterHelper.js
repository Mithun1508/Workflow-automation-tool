...
const isJsonRequest = (curlJson) => {
    if (isContentType(curlJson.headers, ContentTypes.applicationJson))
        return true;
    if (curlJson.data) {
...
const isFormUrlEncodedRequest = (curlJson) => {
    if (isContentType(curlJson.headers, ContentTypes.applicationFormUrlEncoded))
        return true;
...
const isMultipartRequest = (curlJson) => {
    if (isContentType(curlJson.headers, ContentTypes.applicationMultipart))
        return true;
    if (curlJson.files)
        return true;
    return false;
